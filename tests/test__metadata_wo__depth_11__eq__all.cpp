...
TEST_CASE("Deep tree")
{
	...
	SUBCASE("Check range iterator bounds (2)")
	{
		...
		CHECK_EQ((*it).n_white_pawns, 6);
		CHECK_EQ((*it).n_white_knights, 1);
		CHECK_EQ((*it).n_black_knights, 1);

		++it;
		CHECK_EQ((*it).n_white_pawns, 6);
		CHECK_EQ((*it).n_white_knights, 2);
		CHECK_EQ((*it).n_black_knights, 1);

		++it;
		CHECK_EQ((*it).n_white_pawns, 7);
		CHECK_EQ((*it).n_white_knights, 1);
		CHECK_EQ((*it).n_black_knights, 1);
