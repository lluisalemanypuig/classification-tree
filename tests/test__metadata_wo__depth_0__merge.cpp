...
TEST_CASE("Empty trees")
{
	...
	{
		classtree::ctree<data_eq, void> kd2;
...

	{
		CHECK_EQ(kd.size(), 0);
		classtree::ctree<data_eq, void> kd2;
		kd2.add({.i = 1, .j = 1, .k = 1, .z = 1});
		kd2.add({.i = 1, .j = 1, .k = 1, .z = 2});
		kd2.add({.i = 1, .j = 1, .k = 1, .z = 3});
		kd2.add({.i = 1, .j = 2, .k = 1, .z = 1});
		kd2.add({.i = 1, .j = 2, .k = 1, .z = 2});
		kd2.add({.i = 1, .j = 2, .k = 1, .z = 3});
		const size_t r = kd.merge(std::move(kd2));
		CHECK_EQ(r, 6);
		CHECK_EQ(kd.size(), 6);
	}
